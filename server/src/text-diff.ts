// 這是 client 端同名檔案（client/src/lib/text-diff.ts）的雙胞胎，兩邊要一起改；之後有共用層再合併。
...
/**
 * 把 ytext 的內容改成 target，用「最小區間替換」而不是整份重寫。
 * 整份 delete(0, len) + insert 會炸掉所有連線者的游標與 undo。
 * 空文件時這會自然退化成一次 insert。
 */
export function applyTextDiff(ytext: Y.Text, target: string): void {
  const cur = ytext.toString();
  const edit = minimalEdit(cur, target);
  if (!edit) return;
  ytext.doc?.transact(() => {
